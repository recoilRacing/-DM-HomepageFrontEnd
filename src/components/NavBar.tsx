...
const NavBar:React.FC<NavBarProps> = (Props:NavBarProps):JSX.Element => {
    ...
    const [Routes, setRoutes] = useState<Section[]>(
        [
            ...
            {name: "Auto", subURL: "/car"},
            {name: "Partner", subURL: "/partner"},
            {name: "Presse", subURL: "/press"},
            {name: "Unsere Schule", subURL: "/school"},
            {name: "Nachhaltigkeit", subURL: "/sustainability"},
            {name: "Kontakt", subURL: "/contact"}
        ]
    );
    ...
    return <nav className="FlexCenter NavBar" style={{position: Props.position || "fixed"}}>
            ...
            {(dimensions.width < 1160) && <div className="NavBarRight noPadding">
